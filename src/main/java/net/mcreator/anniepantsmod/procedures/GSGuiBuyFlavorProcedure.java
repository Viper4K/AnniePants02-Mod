...
public class GSGuiBuyFlavorProcedure {
		...
		if (entity.getCapability(Capabilities.ItemHandler.ENTITY, null) instanceof IItemHandlerModifiable _modHandlerIter) {
			for (int _idx = 0; _idx < _modHandlerIter.getSlots(); _idx++) {
				...
				if (Items.EMERALD == itemstackiterator.getItem()) {
					emeralds2 = emeralds2 + itemstackiterator.getCount();
				}
			}
		}
		if (emeralds2 >= 5) {
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(AnniepantsmodModItems.ANNIE_GG_FLAVOR.get()).copy();
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal("You have purchased AnniePants02 Flavor!"), false);
		}
	}
}
